...
def get_embedding_for_provider():
    ...
    if config["llm_provider"] == "openai":
        ...
        embeddings = OllamaEmbeddings(
            model=config["embedding"]["model"], base_url=config["llm_base_url"]
        )
        print(
            f"Using OllamaEmbeddings: {config['llm_base_url']}; {config['embedding']['model']}"
        )
    elif config["llm_provider"] == "samba_nova":
        embeddings = SambaNovaCloudEmbeddings(
            model=config["embedding"]["model"],
            sambanova_url=config["llm_base_url"],
            sambanova_api_key=config["samba_nova_api_key"],
        )
        print(
            f"Using SambaNovaCloudEmbeddings: {config['llm_base_url']}; {config['embedding']['model']}"
        )
    else:
        raise Exception(f"Unrecognized llm_provider: {config['llm_provider']}")
...
def create_embeddings_pdf(label, property):
    ...
    with driver.session() as session:
        ...
        for record in result:
            ...
            cypher = (
                cypher
                + " WITH e MATCH (n) WHERE elementId(n) = $id CREATE (n) -[:HAS_EMBEDDING]-> (e)"
            )
